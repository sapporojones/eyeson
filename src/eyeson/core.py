...
def fill_in_object(vic, kill_id, kill_hash):
    ...
    ids_url = "https://esi.evetech.net/latest/universe/names/?datasource=tranquility"
    ids = [vicship_id]
    ids_payload = json.dumps(ids)
    ids_req = requests.post(ids_url, ids_payload)
    ids_json = ids_req.json()
    vic.ship = ids_json[0]["name"]
    return vic


def create_table(vic_list):
    """
    Creates table after being passed a list of Victim objects that have defined attributes
    :param vic_list: A list of Victim objects
    :return: Returns a Rich Table object for printing
    """

    table = Table(title=f"Last {len(vic_list)} kills")
